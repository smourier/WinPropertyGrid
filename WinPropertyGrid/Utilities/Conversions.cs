...
namespace WinPropertyGrid.Utilities
{
    public static class Conversions
    {
        ...
        private static void AppendMessages(StringBuilder sb, Exception? exception, string separator)
        {
            if (exception == null)
                return;
            ...
            AppendMessages(sb, exception.InnerException, separator);
        }
        ...
        public static object? CoerceToEnum(this Type enumType, object input)
        {
            ...
            if (!TryChangeType(input, enumType, out var result))
        ...
        public static bool TryChangeType(object? input, Type conversionType, IFormatProvider? provider, out object? value)
        {
            ...
            if (IsNullable(conversionType))
            {
                ...
                var genericType = conversionType.GetGenericArguments()[0];
                if (TryChangeType(input, genericType, provider, out var vtValue))
                {
                    var nullableType = typeof(Nullable<>).MakeGenericType(genericType);
            ...
            if (conversionType.IsArray || isGenericList)
            {
                if (input is IEnumerable enumerable)
                {
                    ...
                    {
                        count++;
                        if (TryChangeType(obj, elementType!, provider, out var elem))
                        {
                            list.Add(elem);
                        }
                    }
            ...
            if (conversionType == typeof(bool))
            {
                ...
                var svalue = string.Format(provider, "{0}", value).Nullify();
                if (svalue == null)
                    return false;
