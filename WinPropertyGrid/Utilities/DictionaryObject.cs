...
namespace WinPropertyGrid.Utilities
{
    public abstract class DictionaryObject : IDictionaryObject, INotifyPropertyChanged, INotifyPropertyChanging, IDataErrorInfo, INotifyDataErrorInfo
    {
        ...
        protected virtual T? DictionaryObjectGetPropertyValue<T>(T? defaultValue, [CallerMemberName] string? name = null)
        {
            ...
            if (!Conversions.TryChangeType<T>(property.Value, out var value))
                return defaultValue;
        ...
        {
            private readonly DictionaryObject _obj;

            public ObjectComparer(DictionaryObject obj)
            {
                _obj = obj;
            }

            public new bool Equals(object? x, object? y) => _obj.DictionaryObjectAreValuesEqual(x, y);
