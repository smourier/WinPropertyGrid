...
namespace WinPropertyGrid.Utilities
{
    public class DynamicObjectProperty : PropertyDescriptor
    {
        ...
        public virtual object? DefaultValue { get => _defaultValue; set => _defaultValue = Conversions.ChangeType(value, _type); }
        ...
        protected virtual void Construct(string name, Type type, IEnumerable<Attribute> attributes)
        {
            ...
            if (dv != null)
            {
                ...
                _defaultValue = Conversions.ChangeType(dv.Value, _type);
            }
            else
            {
                _defaultValue = Conversions.ChangeType(null, _type);
            }
        }
